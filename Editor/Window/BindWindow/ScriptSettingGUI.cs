    ...
    public partial class BindWindow
    {
        ...
        public void DrawScriptSettingGUI()
        {
            ...
            GUILayout.BeginVertical("box");
            {
                ...
                {
                    if (GUILayout.Button(this.mainSetting.selectScriptSetting.programName))
                    {
                        GenericMenu menu = new GenericMenu(); //初始化GenericMenu 

                        int amount = this.mainSetting.scriptSettingList.Count;
                        for (int i = 0; i < amount; i++)
                        {
                            int scriptIndex = i;
                            menu.AddItem(new GUIContent(this.mainSetting.scriptSettingList[i].programName), false,
                                () => {
                                    this.mainSetting.selectScriptSetting = this.mainSetting.scriptSettingList[scriptIndex];
                                    isSavaSetting = true;
                                });
                        }
                        menu.ShowAsContext();
                    ...
                    if (GUILayout.Button("新建", GUILayout.Width(100)))
                    {
                        ...
                        if (string.IsNullOrEmpty(targetPath) == false)
                        {
                            ...
                            this.mainSetting.selectScriptSetting = scriptSetting;
                            this.mainSetting.scriptSettingList.Add(scriptSetting);
                            scriptSetting.programName = CommonConst.DefaultScriptSettingName;
                    ...
                    {
                        if (this.mainSetting.scriptSettingList.Count > 1)
                        {
                            this.mainSetting.scriptSettingList.Remove(this.mainSetting.selectScriptSetting);
                            AssetDatabase.DeleteAsset(AssetDatabase.GetAssetPath(this.mainSetting.selectScriptSetting));
                            this.mainSetting.selectScriptSetting = this.mainSetting.scriptSettingList.First();
                            isSavaSetting = true;
                        }
                    }
        ...
        {
            ScriptSetting selectSetting = this.mainSetting.selectScriptSetting;
        ...
        void GetSelectMethodData()
        {
            ...
            int variableAmount = this.mainSetting.selectScriptSetting.variableList.Count;
            for (int i = 0; i < variableAmount; i++)
            {
                VariableData variableData = this.mainSetting.selectScriptSetting.variableList[i];
            ...
            int templateAmount = this.mainSetting.selectScriptSetting.templateDataList.Count;
            for (int i = 0; i < templateAmount; i++)
            {
                TemplateData templateData = this.mainSetting.selectScriptSetting.templateDataList[i];
